...
export default function EditorPage() {
  ...
  const performExportAllPages = async () => {
    ...
    const originalScale = { x: stage.scaleX(), y: stage.scaleY() };
    const originalPosition = { x: stage.x(), y: stage.y() };
    const batchSize = 5; // Process pages in batches to manage memory
    ...
    try {
      toast({
        title: 'Exporting...',
        description: `Preparing PDF export with ${totalPages} pages. This may take a moment.`,
      });
      ...
      for (let batchStart = 1; batchStart <= totalPages; batchStart += batchSize) {
        ...
        toast({
          title: 'Exporting...',
          description: `Processing pages ${batchStart}-${batchEnd} of ${totalPages} (${progress}% complete)`,
        });
        ...
        for (let page = batchStart; page <= batchEnd; page++) {
          try {
            ...
            if (page === currentPage) {
              ...
              // Load page image with error handling
              const imgResponse = await fetch(`/api/projects/${params.id}/pages/${page}`);
              if (!imgResponse.ok) {
                console.warn(`Failed to fetch page ${page}: ${imgResponse.statusText}`);
                failedPages++;
                continue;
              }
              ...
              if (!imgData.imageUrl) {
                console.warn(`No image URL for page ${page}`);
                failedPages++;
                continue;
              }
              ...
              try {
                ...
                console.warn(`Failed to load image for page ${page}:`, loadError);
                failedPages++;
                continue;
              }
              ...
              try {
                ...
                console.warn(`Failed to load annotations for page ${page}:`, annotError);
                // Continue without annotations
              }
              
              // Create canvas and render with cleanup
              ...
              if (!ctx) {
                console.warn(`Failed to get canvas context for page ${page}`);
                failedPages++;
                continue;
              }
              ...
              // Cleanup canvas and image references
      ...
      if (pdf && processedPages > 0) {
        pdf.save(`${exportFilename}-all-pages.pdf`);
        const successMessage = failedPages > 0 
          ? `Exported ${processedPages} of ${totalPages} pages as PDF. ${failedPages} pages failed.`
          : `All ${totalPages} pages exported as PDF with annotations.`;
        
        toast({
          title: 'Export successful',
          description: successMessage,
          variant: failedPages > 0 ? 'default' : 'default',
        });
      } else {
      ...
      toast({
        title: 'Export failed',
        description: `Failed to export PDF. ${processedPages > 0 ? `${processedPages} pages were processed before failure.` : 'Please try again.'}`,
        variant: 'destructive',
      });
    } finally {
      stage.scale(originalScale);
                ...
                <ScrollArea className="h-full">
                  <div className="p-3 md:p-4 space-y-4 md:space-y-6">
                    ...
                    <div className="space-y-2 md:space-y-3">
                      ...
                      <Select value={lineStyle} onValueChange={(v: string) => setLineStyle(v as 'solid' | 'dashed' | 'dotted')}>
                    ...
                    <div className="space-y-2 md:space-y-3">
                      ...
                      <Select value={measurementUnit} onValueChange={(v: string) => setMeasurementUnit(v as 'mm' | 'cm' | 'ft')}>
